...
export default function ActivityPage() {
  ...
  const tabClass = (tab: string) => `px-4 py-2 rounded-lg text-sm font-semibold transition-all duration-200 ` +
    (activeTab === tab
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-white tracking-tight">{t('activity.title')}</h1>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: t('activity.stat_open'),         value: openIncidents.length, dot: 'bg-red-400' },
          { label: t('activity.stat_resolved_today'), value: resolvedToday.length, dot: 'bg-emerald-400' },
          { label: t('activity.stat_total'),        value: incidents.length,     dot: 'bg-gray-400' },
          { label: t('activity.stat_monitored'),    value: services.length,      dot: 'bg-blue-400' },
        ].map(stat => (
      ...
      <div className="flex items-center gap-2 border-b border-white/8 pb-3">
        <button className={tabClass('incidents')} onClick={() => setActiveTab('incidents')}>
          {t('activity.tab_incidents')} ({incidents.length})
        </button>
        <button className={tabClass('metrics')} onClick={() => setActiveTab('metrics')}>
          {t('activity.tab_metrics')} ({recentMetrics.length})
        </button>
      </div>
      ...
      {activeTab === 'incidents' && (
        <div className="rounded-2xl border border-white/8 overflow-hidden"
          ...
          <div className="grid grid-cols-5 px-6 py-3 border-b border-white/5">
            {[t('activity.th_service'), t('activity.th_rule'), t('activity.th_severity'), t('activity.th_status'), t('activity.th_start')].map(h => (
              <span key={h} className="text-xs font-semibold text-gray-500 uppercase tracking-wider">{h}</span>
            ))}
          </div>
          {loading ? (
            <div className="px-6 py-10 text-center text-gray-500 text-sm">{t('activity.loading')}</div>
          ) : incidents.length === 0 ? (
            <div className="px-6 py-10 text-center text-gray-500 text-sm">{t('activity.no_incidents')}</div>
          ) : incidents.map((inc, i) => (
            <div key={inc.id}
      ...
      {activeTab === 'metrics' && (
        <div className="rounded-2xl border border-white/8 overflow-hidden"
          ...
          <div className="grid grid-cols-4 px-6 py-3 border-b border-white/5">
            {[t('activity.th_service'), t('activity.th_value'), t('activity.th_metric'), t('activity.th_time')].map(h => (
              <span key={h} className="text-xs font-semibold text-gray-500 uppercase tracking-wider">{h}</span>
            ))}
          </div>
          {loading ? (
            ...
            <div className="px-6 py-10 text-center text-gray-500 text-sm">{t('activity.no_metrics')}</div>
